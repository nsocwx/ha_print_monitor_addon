...
- The ONNX model is downloaded automatically by default from a Hugging Face repository.
- `provider`: `onnx` by default
- `model_path`: Optional override for a custom model file
- `device`: `cpu` | `cuda` | etc.
- `HF_MODEL_REPO`: optional Hugging Face repo override for the default ONNX model
...
class ONNXAnalyzer(ImageAnalyzer):
    ...
    async def analyze_frame(self, image_data: bytes, context: Optional[AnalysisContext]) -> DetectionResult:
        # Run ONNX inference and map model outputs to DetectionResult.
        pass
...
model:
  provider: "onnx"
  model_path: "/data/models/print-failure.onnx"
  device: "cpu"
```

The ONNX analyzer can run anomaly or reconstruction models trained on good prints.
...
- **ONNX**: Use a trained anomaly or detection model exported to ONNX
